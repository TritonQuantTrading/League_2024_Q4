...
namespace QuantConnect
{
    public class ProtectivePutModel : RiskManagementModel
    {
        ...
        private readonly int _maxDaysToExpiration = 150;
        private readonly decimal _minOptionVolume = 30;
        ...
        public override IEnumerable<IPortfolioTarget> ManageRisk(QCAlgorithm algorithm, IPortfolioTarget[] targets)
        {
            ...
            foreach (var kvp in algorithm.Portfolio)
            {
                ...
                {
                    if (algorithm.Portfolio.ContainsKey(existingPutSymbol) && algorithm.Portfolio[existingPutSymbol].Invested)
                    {
                ...
                {
                    targetPrice = currentPrice * 0.95m * (1m + priceReturn);
                ...
                var desiredExpiries = allExpiries
                    .Where(d => (d - algorithm.Time).TotalDays >= _minDaysToExpiration && (d - algorithm.Time).TotalDays <= _maxDaysToExpiration)
                    .OrderBy(d => d)
                    .ToList();
                ...
                var selectedPut = validPuts.OrderBy(p => Math.Abs(p.ID.StrikePrice - targetPrice)).First();
